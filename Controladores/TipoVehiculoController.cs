        ...
        public List<TipoVehiculo> GetAllTipoVehiculo()
        {
            try
            {
                var response = _httpClient
                    .GetAsync($"{_apiBaseUrl}/api/TipoVehiculo/GetAll")
                ...
                var apiResponse = JsonConvert.DeserializeObject<List<TipoVehiculo>>(json);

                return apiResponse ?? new List<TipoVehiculo>();
            }
            catch (Exception)
            {
